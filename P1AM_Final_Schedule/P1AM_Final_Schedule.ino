...
void returnToStart() {
  if (!isMoving) {
    ...
    reverseDistanceTraveled = 0.0;  // Reset reverse distance
    startMovement(true);            // Start reverse movement
  } else {
    // Track reverse distance
    DateTime now = rtc.now();
    unsigned long elapsed = now.unixtime() - movementStartTime;
    reverseDistanceTraveled = MOTOR_SPEED * elapsed;
    ...
    if (reverseDistanceTraveled >= totalDistanceTraveled) {
      ...
      totalDistanceTraveled = 0.0;  // Reset total distance after return
...
void monitorMovement() {
  if (isMoving && !returnTriggered) {  // Only process forward movements
    DateTime now = rtc.now();
    unsigned long elapsed = now.unixtime() - movementStartTime;
