...
void loop() {
  if (!routineComplete) {
    if (!returnTriggered) {
      ...
      static unsigned long lastIterationEndTime = 0;

      if (intervalMillis == 0) {
        intervalMillis = timeInterval * 1000UL;  // Convert to milliseconds
      ...
      if (!isMoving) {
        if (currentRepeat < repeats) {
          // Check if the interval has passed since the last iteration ended
          if (millis() - lastIterationEndTime >= intervalMillis || currentRepeat == 0) {
            executeRoutine();
            lastIterationEndTime = millis();  // Record the end time of the iteration
            currentRepeat++;
          }
        } else {
...
void returnToStart() {
  if (!isMoving) {
    startMovement(true);  // Move in reverse
  } else {
    monitorMovement();
    if (!isMoving) {
      returnTriggered = false;
...
void stopMovement() {
  ...
  P1.writeAnalog(0, ANALOG_SLOT, ANALOG_OUTPUT_CHANNEL1);  // Turn off analog output
  isMoving = false;
  currentPayout = 0.0;
}
